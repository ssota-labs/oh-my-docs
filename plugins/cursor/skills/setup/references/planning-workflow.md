...
4. Give every artifact a stable, kind-prefixed ID and register its filename in
   the sibling `meta.json`.
5. Set lifecycle states for implementation readiness:
   - PRD: `status: active`
   - Specification: `stage: accepted`
   - Plan: `stage: ready` (or `active` while implementing)
...
7. Validate with `omdocs check` or `pnpm check:planning`.
