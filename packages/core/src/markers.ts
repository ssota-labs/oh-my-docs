...
const MARKER_BLOCK =
  /<!--\s*oh-my-docs:start\s*-->[\s\S]*?<!--\s*oh-my-docs:end\s*-->/g;

export function hasMarkerBlock(source: string): boolean {
  MARKER_BLOCK.lastIndex = 0;
  return MARKER_BLOCK.test(source);
}

export function wrapMarkerBlock(body: string): string {
...
export function mergeMarkerBlock(
...
): { readonly content: string; readonly kind: 'create' | 'update' | 'merge' | 'skip' } {
  ...
  if (hasMarkerBlock(existing)) {
    ...
    if (next === existing) return { content: existing, kind: 'skip' };
