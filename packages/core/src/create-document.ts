...
import { collectPlanningDocuments, validatePlanning, type DocumentKind } from './planning.ts';
import type { FileOperation } from './types.ts';

export type NewDocumentKind = DocumentKind;

export interface CreateDocumentOptions {
  readonly cwd: string;
  readonly kind: NewDocumentKind;
...
const KIND_DIR: Readonly<Record<NewDocumentKind, string>> = {
  prd: 'planning/prds',
...
const KIND_FILE_PREFIX: Readonly<Record<NewDocumentKind, string>> = {
  prd: 'prd',
  story: 'us',
  spec: 'spec',
  plan: 'plan',
...
const KIND_ID_PREFIX: Readonly<Record<NewDocumentKind, string>> = {
  prd: 'PRD-',
  story: 'US-',
  spec: 'SPEC-',
  plan: 'PLAN-',
...
const TEMPLATE_FILE: Readonly<Record<NewDocumentKind, string>> = {
  prd: 'prd.mdx',
...
function stripKindPrefix(slug: string, kind: NewDocumentKind): string {
  const filePrefix = `${KIND_FILE_PREFIX[kind]}-`;
  const idPrefix = KIND_ID_PREFIX[kind].toLowerCase();
  let next = slug;
  if (next.startsWith(filePrefix)) next = next.slice(filePrefix.length);
...
function resolveDocsPath(cwd: string, explicit?: string): string {
...
function readTemplate(docsPathAbsolute: string, kind: NewDocumentKind): string {
  const candidates = [
  ...
  throw new Error(
    `Template for ${kind} not found. Looked under ${docsPathAbsolute}/templates.`,
  );
}

function replaceFrontmatterField(source: string, field: string, value: string): string {
  const pattern = new RegExp(`^(${field}:\\s*).*$`, 'm');
  if (pattern.test(source)) return source.replace(pattern, `$1${value}`);
  return source.replace(/^---\n/, `---\n${field}: ${value}\n`);
}

function renderDocument(template: string, kind: NewDocumentKind, id: string, title: string): string {
  let body = template;
...
function registerMeta(metaSource: string, slug: string): string {
  const meta: unknown = JSON.parse(metaSource);
  if (typeof meta !== 'object' || meta === null || Array.isArray(meta)) {
    throw new Error('meta.json must be a JSON object');
  }
  const record = meta as { pages?: unknown; [key: string]: unknown };
...
function validateAfterCreate(
  contentDirectory: string,
  staged: {
    readonly relativeFile: string;
    readonly content: string;
    readonly metaRelative: string;
 ...
 */
export function planCreateDocument(options: CreateDocumentOptions): CreateDocumentResult {
  ...
  const relativePath = join(docsPath, 'content/docs', KIND_DIR[options.kind], `${slug}.mdx`).replace(
    /\\/g,
    '/',
  );
  const metaRelative = join(docsPath, 'content/docs', KIND_DIR[options.kind], 'meta.json').replace(
    /\\/g,
    '/',
  );
