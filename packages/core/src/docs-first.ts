import { parseFrontmatter } from './planning.ts';
...
export interface DocsFirstInput {
  readonly changedPaths: readonly string[];
  readonly prBody: string;
  readonly readBaseFile: (path: string) => string;
  /**
   * When false, the docs-first gate is being introduced in this change and
   * enforcement is skipped until a later PR sees the gate on its base SHA.
   */
  readonly gatePresentOnBase?: boolean;
}

export function extractPlanPath(prBody: string): string | null {
...
export function isDocumentationOnlyPath(path: string): boolean {
  ...
  if (DOCUMENTATION_ONLY_FILES.has(normalized)) return true;
  return DOCUMENTATION_ONLY_PREFIXES.some((prefix) => normalized.startsWith(prefix));
}

function coveredBy(path: string, codeAreas: readonly string[]): boolean {
...
export function gateScriptExistsOnBase(readBaseFile: (path: string) => string): boolean {
  for (const candidate of [
    'scripts/check-docs-first.ts',
    'scripts/check-docs-first.mjs',
    'packages/core/src/docs-first.ts',
  ]) {
    try {
 ...
 */
export function validateDocsFirst(input: DocsFirstInput): readonly string[] {
  ...
  if (data.stage !== 'ready' && data.stage !== 'active') {
    problems.push(
      `${planPath}: base plan stage must be ready or active (found ${String(data.stage ?? '?')})`,
    );
  }

  const codeAreas = Array.isArray(data.codeAreas)
    ? data.codeAreas.filter((area): area is string => typeof area === 'string' && area.length > 0)
    : [];
